...
最近看了go-zero的源码,里面一个特别牛的工具,goctl,理念也是让使用者关注业务逻辑的开发,各个端的代码一键生成,这远比复制粘贴写100个1000个api强多了。因为仅仅复制粘贴从技术能力和开发思维上来看其实是没成长的,怎么快速提升开发效率并且还不会出错更值得深思。
  
1.如果任务需要对中间的数据做处理如何可配置？比如名称清洗
  是不是可以考虑将每一个任务的代码生成出来,当需要特殊处理的时候由开发者编写自己的函数进行处理?
